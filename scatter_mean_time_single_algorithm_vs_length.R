...
for (i in 1:number_unique_algorithms) {
    ...
    plot <- ggplot(scatter_data_frame[scatter_data_frame[["algorithm"]] == unique_algorithm,])
    ...
    plot <- plot + geom_smooth(aes(x = length, y = mean), color="black", alpha=0.25)
    plot <- plot + labs(x = "Length of Input", y = "Mean Processing Time (ms)", title = paste("Mean Processing Time per Algorithm vs Input Length for", input_file_name_full, "and algorithm", unique_algorithm))
    plot <- plot + fte_theme()
    plot <- plot + theme(legend.position="none")

    ggsave(paste("graphs/scatter_mean_vs_input_length_", unique_algorithm, ".png", sep = ""), dpi = 1200, width = 8, height = 6, type = "cairo")
}

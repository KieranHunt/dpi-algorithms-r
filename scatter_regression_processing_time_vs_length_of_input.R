...
unique_algorithms <- c("Horspool", "QuickSearch", "NotSoNaive", "RabinKarp")
...
for (i in 1:length(unique_algorithms)) {
    ...
    fit <- lm(log(sub_data_frame[["elapsed"]]) ~ sub_data_frame[["inputLength"]], data = sub_data_frame)
    print(algorithm)
    print(summary(fit))
    reg_alpha <- (fit[["coefficients"]])[1][["(Intercept)"]]
    reg_beta <- (fit[["coefficients"]])[2][['sub_data_frame[["inputLength"]]']]

    sub_data_frame[["predicted"]] <- predict(fit)

    plot <- ggplot()
    plot <- plot + geom_point(data = sub_data_frame, aes(x = inputLength, y = elapsed, color = -found), alpha=0.50)
    plot <- plot + geom_line(data = sub_data_frame, aes(x = inputLength, y = exp(predicted)))
    plot <- plot + labs(x = "Input Length", y = "Processing Time (ms)", title = paste("Processing Time vs Input Length for", algorithm))
    plot <- plot + annotate("text", x = 0, y = Inf, vjust = 2, hjust = 0, label = sprintf("f(x) = e^(%f + %fx)", reg_alpha, reg_beta))
    plot <- plot + theme(legend.position="none")

    ggsave(paste("graphs/scatter_elapsed_vs_input_length_", algorithm, ".png", sep = ""), dpi = 1200, width = 8, height = 6, type = "cairo")
